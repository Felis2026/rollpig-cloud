...
- **烤箱补货**：保存群日活与投票申请，并在单个事务中完成有效成员过滤、群管理双票验票和普通烧烤配额恢复。
...
Cloud `0.8.0` 可以先于客户端升级，旧版 Plus 的既有玩法保持不变。使用补签时，先升级 Cloud，再升级 Plus 至 `0.14.1`。新版 Plus 连接旧 Cloud 仍可查看已有昨日记录，缺少记录时会提示升级，不会回退本地补签。本次补签接口复用现有 JSON 快照，无需数据库迁移。
